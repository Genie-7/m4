...
#ref https://github.com/bknyaz/graph_attention_pool/blob/master/extract_superpixels.py
...
def parse_args():
    ...
    parser.add_argument('-D', '--dataset', type=str, default='mnist', choices=['mnist', 'cifar10', 'fashionmnist'])
...
if __name__ == '__main__':
    ...
    if args.dataset == 'mnist':
        ...
        data = datasets.FashionMNIST(args.data_dir, train=is_train, download=True)
        assert args.compactness < 10, ('high compactness can result in bad superpixels on FashionMNIST')
        assert args.n_sp > 1 and args.n_sp < 28*28, (
            'the number of superpixels cannot exceed the total number of pixels or be too small')
    else:
