...
class SuperPixDGL(torch.utils.data.Dataset):
    def __init__(self,
                 ...
                 use_mean_px=True,
                 use_coord=True):
        print(f"Initializing SuperPixDGL for {dataset} {split}")
        self.split = split
        self.graph_lists = []

        if dataset == 'FashionMNIST':
            self.img_size = 28
            # Only look for the main .pkl file
            file_name = f'fashionmnist_75sp_{split}.pkl'
...
#         """
#             Takes input standard image dataset name (MNIST/CIFAR10/FashionMNIST)
...
#         """
#         print("eerge in SuperPixDatasetDGL init")
#         t_data = time.time()
#         self.name = name
...
#         use_coord = True
#         self.test = SuperPixDGL("/Users/brendangignac/Documents/Summer2024/research/m4/SuperpixelGeneration/data", dataset=self.name, split='test',
#                                 use_mean_px=use_mean_px,
#                                 use_coord=use_coord)

#         self.train_ = SuperPixDGL("/Users/brendangignac/Documents/Summer2024/research/m4/SuperpixelGeneration/data", dataset=self.name, split='train',
#                                   use_mean_px=use_mean_px,
#                                   use_coord=use_coord)
...
class SuperPixDatasetDGL(torch.utils.data.Dataset):
    def __init__(self, name, data_dir, num_val=5000):
        print(f"Initializing SuperPixDatasetDGL for {name}")
        t_data = time.time()
        self.name = name
        ...
        # Load test set
        self.test = SuperPixDGL(data_dir, dataset=self.name, split='test',
                                use_mean_px=use_mean_px,
                                use_coord=use_coord)

        # Load full training set
        self.train_ = SuperPixDGL(data_dir, dataset=self.name, split='train',
                                  use_mean_px=use_mean_px,
                                  use_coord=use_coord)
...
class SuperPixDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, name):
        ...
        """
        print("eerge in SuperPixDataset init")
        start = time.time()
        print("[I] Loading dataset %s..." % (name))
        self.name = name
        data_dir = '/Users/brendangignac/Documents/Summer2024/research/m4/SuperpixelGeneration/data/'
...
def save_dataset_as_pt(dataset, split_name, save_dir):
    ...
    }, f"{save_dir}/{split_name}.pt")
    print(f"Saved {split_name} dataset to {save_dir}/{split_name}.pt")

if __name__ == "__main__":
    print("Running main")
    dataset = "FashionMNIST"
    #Will have to update between machines - make a env var 
    data_dir = "/Users/brendangignac/Documents/Summer2024/research/m4/SuperpixelGeneration/data"
    save_dir = "/Users/brendangignac/Documents/Summer2024/research/m4/superpixelProcessing"
    
    print(f"Initializing dataset from {data_dir}")
    superpix_dataset = SuperPixDatasetDGL(name=dataset, data_dir=data_dir)
    
    print("Processing and saving data...")
    save_dataset_as_pt(superpix_dataset.train, "train", save_dir)
    save_dataset_as_pt(superpix_dataset.val, "val", save_dir)
    save_dataset_as_pt(superpix_dataset.test, "test", save_dir)
